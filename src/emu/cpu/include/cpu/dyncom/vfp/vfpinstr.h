...
VSTR_INST : {
    if ((inst_base->cond == ConditionCode::AL) || CondPassed(cpu, inst_base->cond)) {
        ...
        if (inst_cream->single) {
            ...
            const std::uint32_t word1 = cpu->ExtReg[inst_cream->d * 2 + 0];
            const std::uint32_t word2 = cpu->ExtReg[inst_cream->d * 2 + 1];
    ...
    if ((inst_base->cond == ConditionCode::AL) || CondPassed(cpu, inst_base->cond)) {
        ...
        for (unsigned int i = 0; i < inst_cream->regs; i++) {
            if (inst_cream->single) {
                ...
                const std::uint32_t word1 = cpu->ExtReg[(inst_cream->d + i) * 2 + 0];
                const std::uint32_t word2 = cpu->ExtReg[(inst_cream->d + i) * 2 + 1];
    ...
    if ((inst_base->cond == ConditionCode::AL) || CondPassed(cpu, inst_base->cond)) {
        ...
        for (unsigned int i = 0; i < inst_cream->regs; i++) {
            if (inst_cream->single) {
                ...
                const std::uint32_t word1 = cpu->ReadMemory32(addr + 0);
                const std::uint32_t word2 = cpu->ReadMemory32(addr + 4);
    ...
    if (inst_base->cond == ConditionCode::AL || CondPassed(cpu, inst_base->cond)) {
        ...
        for (unsigned int i = 0; i < inst_cream->regs; i++) {
            if (inst_cream->single) {
                ...
                const std::uint32_t word1 = cpu->ReadMemory32(address + 0);
                const std::uint32_t word2 = cpu->ReadMemory32(address + 4);
